...
class GenerateErrorPagesCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $templatePath = __DIR__ . '/../../templates/error_base.html.twig';
        $translationPath = __DIR__ . '/../../translations/messages.' . $lang . '.yaml';
        $outputDir = getcwd() . '/templates/bundles/TwigBundle/Exception/';

        if (!file_exists($templatePath)) {
            $output->writeln("<error>Template de base introuvable : $templatePath</error>");
            return Command::FAILURE;
        }
        if (!file_exists($translationPath)) {
            $output->writeln("<error>Fichier de langue introuvable : $translationPath</error>");
            return Command::FAILURE;
        }
